...
class ObjectsBar(QTabWidget):
    ...
    def file_tab(self):
        ...
        export_script.clicked.connect(self.fsm_model.export)
    ...
    def object_tab(self):
        ...
        addCircle.clicked.connect(
            lambda: self.fsm_model.instant_add_object_to_curr(ICircle())
        )

        addSquare = QPushButton("add square")
        addSquare.clicked.connect(
            lambda: self.fsm_model.instant_add_object_to_curr(ISquare())
        )

        addStar = QPushButton("add star")
        addStar.clicked.connect(
            lambda: self.fsm_model.instant_add_object_to_curr(IStar())
        )

        addTriangle = QPushButton("add triangle")
        addTriangle.clicked.connect(
            lambda: self.fsm_model.instant_add_object_to_curr(ITriangle())
        )

        addMarkupText = QPushButton("add text")
        addMarkupText.clicked.connect(
            lambda: self.fsm_model.instant_add_object_to_curr(
                IMarkupText(
                    """click to add text"""
        ...
        addMathTex.clicked.connect(
            lambda: self.fsm_model.instant_add_object_to_curr(
                IMathTex(r"\xrightarrow{x^6y^8}", fsm_model=self.fsm_model)
            )
        )
